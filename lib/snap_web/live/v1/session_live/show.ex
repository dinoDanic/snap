defmodule SnapWeb.V1.SessionLive.Show do
  alias Snap.Panes
  alias Snap.Windows
  alias Snap.Repo
  ...
  def render(assigns) do
    ~H"""
    <.svelte class="h-full" name="v1/pane/pane-list/pane-list" props={%{panes: @panes}} />
    """
  end
  ...
  def handle_params(%{"id" => session_id}, _, socket) do
    ...
    case session.windows do
      [first_window | _] ->
        ...
        {:noreply, push_patch(socket, to: "/v1/session/#{session_id}/window/#{window_id}")}

      [] ->
        case Windows.create_window(%{name: "welcome_window"}, session) do
          {:ok, window} ->
            {:noreply, push_patch(socket, to: "/v1/session/#{session.id}/window/#{window.id}")}

          {:error} ->
            {:noreply, push_patch(socket, to: "/v1/session/")}

          _ ->
            {:noreply, push_patch(socket, to: "/v1/")}
        end
    end
  end
        ...
        case active_window.panes do
          [] ->
            case Panes.create_pane(%{name: "Untitled"}, active_window) do
              {:ok, _pane} ->
                {:noreply,
                 push_navigate(socket, to: "/v1/session/#{session_id}/window/#{window_id}")}
  ...
  def handle_event("list_sessions", _unsigned_params, socket) do
    {:noreply, push_navigate(socket, to: "/v1/session/list")}
  end
  ...
  def handle_event("create_window", %{"window_name" => window_name}, socket) do
    ...
    case Windows.create_window(%{name: window_name}, session) do
      {:ok, window} ->
        {:noreply, push_patch(socket, to: "/v1/session/#{active_session.id}/window/#{window.id}")}
